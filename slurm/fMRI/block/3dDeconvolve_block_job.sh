...
if [ -f $TIMING_POS ] && [ ! -f block_deconv+orig.BRIK ]
    then
...
if [ -f block_deconv+orig.BRIK ] && \
[ ! -f block_deconv_blur5+orig.BRIK ]
    then
			echo "I found something to blur"
        ${AFNI_BIN}/3dmerge -prefix block_deconv_blur5 -1blur_fwhm 5.0 -doall block_deconv+orig
fi
